...
describe('Attendance - Call Out Lists', () => {
  ...
  it('Updates a call out list', () => {
    cy.get(
      `#callOuts--searchResults a[data-cy="${newCallOutListName}"]`
    ).click()
  ...
  it('Adds call out list members', () => {
    ...
    cy.get('#container--callOutListMembers a[data-employee-number]').should(
      'not.exist'
    )
    ...
    cy.get(
      '.modal #container--callOutListAvailableEmployees a[data-employee-number]'
    )
      .first()
      .click()
      .should('not.exist')
  ...
  it('Prints a call out list', () => {
    ...
    cy.get("#tab--callOuts-reports a[download][href*='/reports/']").each(
      ($reportLink) => {
