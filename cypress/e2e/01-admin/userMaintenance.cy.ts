...
describe('Admin - User Maintenance', () => {
  ...
  it('Can toggle "Can Log In"', () => {
    cy.get(
      `#tbody--users tr[data-user-name="${newUserName}"] select[data-field="canLogin"]`
    )
      .select(0)
      .wait(250)
      .select(1)
  ...
  it('Can toggle "Is Admin"', () => {
    cy.get(
      `#tbody--users tr[data-user-name="${newUserName}"] select[data-field="isAdmin"]`
    )
      .select(1)
      .wait(250)
      .select(0)
  ...
  it('Can update permissions', () => {
    cy.get(
      `#tbody--users tr[data-user-name="${newUserName}"] button[data-cy="permissions"]`
    ).click()
