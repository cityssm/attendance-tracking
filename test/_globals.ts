...
export const testAdmin = (config.tempUsers ?? []).find(
  (possibleUser) => possibleUser.user.canLogin && possibleUser.user.isAdmin
)
...
export const testUser = (config.tempUsers ?? []).find(
  (possibleUser) =>
