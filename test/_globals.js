...
export const testAdmin = (config.tempUsers ?? []).find((possibleUser) => possibleUser.user.canLogin && possibleUser.user.isAdmin);
if (testAdmin === undefined) {
    console.error('No testAdmin user available.');
}
export const testUser = (config.tempUsers ?? []).find((possibleUser) => possibleUser.user.canLogin &&
